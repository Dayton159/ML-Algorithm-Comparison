...
def efficiencyTest():
    ...
    bookRandomForest = RandomForestClassifier(n_estimators=60, random_state=0)

    bookRandomForest.fit(X_train, y_train)

    predicted = bookRandomForest.predict(X_test)

    print(predicted)

    print(classification_report(y_test, predicted))

    print("\nRandomForest's Accuracy: ", metrics.accuracy_score(y_test, predicted))
    # precision tp / (tp + fp)
    print("\n RandomForest's Precision: ", metrics.precision_score(y_test, predicted, average="macro"))
    # recall: tp / (tp + fn)
